...
# ==================== RESULT ENTRY WINDOW ====================
# This class creates the Result screen where user can add student marks and calculate percentage.
# User can select registered student roll number, fetch student name & course, then submit result data.
class ResultClass:
    def __init__(self,root):
        ...
        btn_search=Button(self.root,text="Search",font=("times new roman",15),bg="#03a9f4",activebackground="#03a9f4",cursor="hand2",command=self.search).place(x=500,y=100,width=100,height=28)
        ...
        #=========content_window========
        self.bg_img=Image.open("assets/result.png")
     ...
     # =========================================
    def fetch_roll(self):
        """Load student roll numbers from student table into result dropdown list."""
    ...
    def search(self):
        """Fetch selected student details (name, course) into form by roll number."""
        con=sqlite3.connect(database="rms.db")    
        cur=con.cursor()
        try:
    ...
    def add(self):
        """Insert student result record into database after validation."""
        con=sqlite3.connect(database="rms.db")    
        cur=con.cursor()
        try:
